...
void init_log_ui() {
    ...
    gtk_widget_set_vexpand(topScrollWindow, TRUE); // 수직 확장 가능
    gtk_container_add(GTK_CONTAINER(topScrollWindow), directoryListBox);

    GtkWidget *topArea = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5); // 상단 영역 박스
    ...
    GtkWidget *scrollWindow = gtk_scrolled_window_new(NULL, NULL); // 로그 창 스크롤
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrollWindow), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    gtk_container_add(GTK_CONTAINER(scrollWindow), logTextView);

    GtkWidget *bottomArea = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5); // 하단 영역 박스
